...
whiting[["CA"]]$Maturity = revalue(whiting[["CA"]]$Maturity, c("A" = "0", "B" = "1", "Ba" = "1", "Bb" = "1", "C" = "1", "D" = "1", "E" = "1", "F" = "66", "61"="0", "62"="1", "63"="1", "64"="1", "65"="1", "1"="0", "2"="1", "3"="1", "4"="1", "5"="1", "I"="0", "M"="1"))
...
dataL<-ddply(dataG,.(haul.id,length), summarise,nb = sum(nb),year=unique(year),area=unique(area))
dataL$rg<-rep(-1,dim(dataL)[1])
#summary(dataL)

wL1<-droplevels(subset(wL,haul.id %in% dataL$haul.id))
wL1$length<-as.numeric(sub('.([^,]+),.*', '\\1', wL1$sizeGroup)) # new  get length from intervals



for(i in 1:dim(dataL)[1]){
  ww<-subset(subset(wL1,haul.id==dataL$haul.id[i]),length==dataL$length[i])  #new
  
  dataL$rg[i]<-dataL$nb[i]/max(dataL$nb[i],ww$Freq)    #new
...
for(i in 1:dim(dataG)[1]){
  ...
  ww<-subset(subset(wL1,haul.id==dataG$haul.id[i]),length==dataG$length[i])  #new
  
  dataG$rg[i]<-max(dataG$nb[i],ww$Freq)/dataG$nb[i]  #new
...
write.taf(dataG,"mat_dataG2.csv", dir="data")
...
a50fun<-function(d,indices) {
  ...
  res<-c(A50.91,A50.92,A50.93,A50.94,A50.95,A50.96,A50.97,A50.98,A50.99,A50.00,A50.01,A50.02,
         ...
         A50.15,A50.16,A50.17,A50.18,A50.19,A50.20,A50.21,A50.22)
  return(res)
}
...
with(rezboot,plot(year,A50,pch=16,ylim=c(1,2),xlim=c(1980,fyear),xaxt="n",xlab="",ylab="Age at 50% Maturity"))
